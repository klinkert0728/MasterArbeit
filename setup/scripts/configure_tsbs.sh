...
### Generate Data
tsbs_generate_data --use-case=devops --seed=123 --scale=800  --timestamp-start="2024-10-26T00:00:00Z" --timestamp-end="2024-10-27T00:00:00Z" --log-interval="60s" --format="victoriametrics" > ~/data.txt

### Generate Queries
# Keep in mind the timestamp start and end should be the same as the data generated.
tsbs_generate_queries --use-case=devops --seed=123 --scale=800 --timestamp-start="2024-10-26T00:00:00Z" --timestamp-end="2024-10-27T00:00:00Z" --queries=1440 --query-type=double-groupby-all --format=victoriametrics > ~/queries.txt

mkdir -p $HOME/results

### Load data into sut
tsbs_load_victoriametrics --urls="http://$SUT_IP_AND_PORT_LATEST/write" --workers=10 --batch-size=100 --file="$HOME/data.txt" --latencies-file=$HOME/results/latenciesInserts.csv 2>&1 | tee $HOME/results/logInserts.log & tsbs_load_victoriametrics --urls="http://$SUT_IP_AND_PORT_OTHER/write" --workers=10 --batch-size=100 --file="$HOME/data.txt" --latencies-file=$HOME/results/latenciesInserts_other.csv 2>&1 | tee $HOME/results/logInserts_other.log